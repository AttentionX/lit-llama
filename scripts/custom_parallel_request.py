            ...
            if not queue_of_requests_to_retry.empty():
                ...
                try:
                    ...
                    if task_index >= len(tasks):
                        ...
                        break
                    request_json = tasks[task_index]

                    # print(request_json)
                    title = request_json['title']
                    print(f'Getting pdf text for {title}')
                    start_time = datetime.now()
                    pages = prepare_arxiv.retrieve_pdf_from_url(request_json['url'])
                    end_time = datetime.now()
                    print(f'Finished getting pdf text for {title} in {end_time-start_time}')

                    original_paper_text = '\n\n'.join(pages)
                    paper_text = openai_api.get_chunk_tokens(original_paper_text, max_tokens=7000, model=engine)
                    request_json['paper_text'] = paper_text

                    mode = 'w'
                    text_save_path = request_json['save_filepath'].replace('.jsonl', '.txt').replace('qa','raw_text')
                    with open(text_save_path, mode) as f:
                        f.write(f'{paper_text}')
...
class APIRequest:
    ...
    async def call_api(
        ...
        if error:
            ...
            print(f"Saved {title} to {save_filepath}")
