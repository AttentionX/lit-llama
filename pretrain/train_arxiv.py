...
def load_datasets(data_dir: str = "data/pretrain_dataset") -> Tuple[np.ndarray, np.ndarray]:
    train_data = np.memmap(os.path.join(data_dir, "GPT-4 Technical Report_raw.bin"), dtype=np.uint16, mode="r")
    val_data = np.memmap(os.path.join(data_dir, "GPT-4 Technical Report_raw.bin"), dtype=np.uint16, mode="r")
